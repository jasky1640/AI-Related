    ...
    class Position:
        def __init__(self, board, seeds, turn):
            ...
            if len(seeds) != board.size:
                raise ValueError('mismatch between size of seeds list and size of board: %d vs %d' % len(seeds),
                                 board.size)
        ...
        def is_initial(self):
            ''' Determines if this position is the initial position.
            '''
        ...
        def is_capture(self, pit):
            ''' Determines if the given move from this position is a
                capturing move.
            ...
            return self._board.owner[last] == self._turn and self._seeds[last] == 0 and self._board.opposite[
                last] is not None and self._seeds[self._board.opposite[last]] > 0

        def is_move_again(self, pit):
            ''' Determines if the given move from this position results
                in a free move.
        ...
        def result(self, p):
            ...
            if succ._seeds[last] == 1 and succ._board.opposite[last] is not None and succ._seeds[
                succ._board.opposite[last]] > 0 and succ._board.owner[last] == self._turn:
        ...
        def __eq__(self, other):
            return isinstance(other,
                              self.__class__) and self._seeds == other._seeds and self._turn == other._turn and self._board is other._board


if __name__ == '__main__':
    board = Kalah(6)
    pos = board.initial_position(4)
    print(pos)
